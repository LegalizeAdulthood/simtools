/* absolute I/O support for Windows
 *
 * Based on unxabsio.c by Paul Koning
 *
 * Note that this particular version of absio is mostly a dummy, since
 * there is very little special that has to be done on Windows for absolute
 * I/O.  This file serves as the Windows equivalent of unxabsio.c.
 */
 ...
 *
 * For Windows, the _stat function doesn't return a size for raw devices
 * (it returns zero) but the stdio routines are otherwise perfectly
 * suitable.  So this routine does NOT set absflag since no special abs I/O
 * routines are needed.  It does check the return from _stat(); if this
 * indicates a character device, it checks that rsize has been supplied and
 * complains if not.
 */
...
void absname (const char *rname)
{
	...
	if (_stat (rname, &sbuf)) return;	/* try to get info about device */
	diskblocks = UP(sbuf.st_size, BLKSIZE) / BLKSIZE;
	if (diskblocks == 0) {
		if (sbuf.st_mode & _S_IFCHR) {
			diskblocks = rsize;
			if (rsize == 0) {
